...
class Interface():
    ...
    def create_main_window(self):
        ...
        # Diferents opcions
        self.mAltas = tk.Menu(self.barraMenu, tearoff=0)
        ...
        #Comandos
        self.mAltas.add_command(label="Alta de pacient", command= self.alta_paciente)
        self.mAltas.add_command(label="Alta de metge")
        ...
        self.mSalir.add_command(label= "Sortir", command= self.v.destroy)

        #Agregar menus a la barra
        self.barraMenu.add_cascade(label = "Altes", menu= self.mAltas)
    ...
    def alta_paciente(self):
        ...
        etiq_0= tk.Label(v_ingreso, text= "Inserir dades del pacient nou:")
        ...
        spin_sang = ttk.Combobox(v_ingreso, values=grup_sanguini)
        ...
        alta_pac_params=partial(self.alta_pac_aux, v_nom, v_dir, v_ciutat, v_cp, v_tlf, v_email, spin_sang, v_ingreso)

        # Programar botó
        btnAsignar=tk.Button(v_ingreso,text="Asignar", command = alta_pac_params).grid(column=0,row=8)
        btnSortir=tk.Button(v_ingreso,text="Sortir", command = v_ingreso.destroy).grid(column=1,row=8)
    ...
    def alta_pac_aux(self, nom, dire, ciutat, cp, tlf, email, sang, v_ingreso):
        ...
        if not all([nom.get(), dire.get(), ciutat.get(), cp.get(), tlf.get(), email.get(), sang.get()]):
