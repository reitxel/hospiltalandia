...
#            return recep
    ...
    def alta_paciente(self):
        ...
        #Prepara la finestra
            ...
            alta_pac_params=partial(self.alta_pac_aux, v_nom, v_dir, v_ciutat, v_cp, v_tlf, v_email, spin_sang, recep, v_ingreso)# PASO LA FUNCIO I TOTS ELS PARAMETRES QUE VULL QUE TINGUI LA FUNCIO, AIXO SI QUE HO PUC POSAR AL COMMAND
        ...
        def alta_pac_aux(self, nom, dire, ciutat, cp, tlf, email, sang,recep, v_ingreso,):
            """
        ...
        if not all([nom.get(), dire.get(), ciutat.get(), cp.get(), tlf.get(), email.get(), sang.get()]):# NOMES SEXECUTA AL CLICAL AL BOTO
            ...
            self.Hospital.alta_pac(nom.get(), dire.get(), ciutat.get(), cp.get(), tlf.get(), email.get(), sang.get(),recep)
    ...
    def alta_medico(self):
        ...
        # Grup sanguini (desplegable)
        etiq_especialidad = tk.Label(v_ingreso, text= "Especialidad:")
        etiq_especialidad.grid(column=0, row=7)
        v_especialidad = tk.StringVar()
        v_especialidad.set("")
        e_especialidad = tk.Entry(v_ingreso, textvariable=v_especialidad)
        e_especialidad.grid(column=1, row=6)
